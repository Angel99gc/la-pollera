...
export const LayoutModal = ({ children, title, onClose }: { children: ReactNode, title?: string, onClose: () => void }) => {
  ...
  return (
    ...
    >
      <div
        ref={ref}
        className={`flex flex-col relative w-full sm:w-auto sm:min-w-sm max-w-full max-h-[98dvh] rounded-lg overflow-hidden bg-white border border-gray-100 shadow-sm `}
      >
        ...
        <Divider />
        {/* <div className="overflow-y-auto bg-gray-100 rounded-lg p-2"></div> */}
        {children}
      </div>
    </div>
  )
